...
test('Predicates THIS', t => {
  ...
  t.equal(both({foo: {bar: true}}), false, 'combination - and: undefined');
  ...
  t.equal(either({foo: {baz: true}}), true, 'combination - or: undefined');
});
...
test('Predicates NOT with undefined', t => {
  ...
  t.equal(EQ({foo: {baz: 2}}), true, 'equal');
  t.equal(notEQ({foo: {baz: 2}}), false, 'not equal');
  t.equal(notEQ({foo: {baz: 4}}), true, 'not not equal');
  t.equal(notEQ({bar: true}), false, 'not not equal');
});
...
test('Predicates', t => {
  ...
  t.equal(getPredicate('or', [() => true, () => false])(1), true, 'or: true early');
  t.equal(getPredicate('or', [() => false, () => true])(1), true, 'or: true late');
  t.equal(getPredicate('or', [() => false, () => false])(1), false, 'or: false');

  t.equal(getPredicate('and', [() => false, () => true])(1), false, 'and: false early');
  t.equal(getPredicate('and', [() => true, () => false])(1), false, 'and: false late');
  t.equal(getPredicate('and', [() => true, () => true])(1), true, 'and: true');
  t.equal(getPredicate('and', [() => true, () => undefined])(1), false, 'and: handle undefined');
